...
data_clean_1950 %>%
  ...
  mutate(proportion = percent(n/nrow(data_clean_1950)))
```
...
data_clean_1950 %>%
  ...
  mutate(n = row_number(),
         female_percent = percent(female_percent)) %>% 
  filter(n < 6 | n > (nrow(.) - 5)) %>%
  dplyr::select(-n)
```

A look at the major nations shows that the USA has been leading the pack for a while, with GBR in second.
